    ...
    with open('pride-and-prejudice.txt', 'r', encoding="utf-8") as f:
        ...
        for line in f:
            ...
            for x in line.split():
                ...
                added = new_tree.add(x)
                end_time = time.time()
                if added:
                    add_time.append((end_time - start_time) * (10**9))
                    num_words.append(new_tree.size(new_tree.root))
    Total_Add_Average.append(statistics.stdev(add_time))
    print("Experiment--", i+1)
    print("The size of the binary tree set is", new_tree.size(new_tree.root))
    ...
    with open('words-shuffled.txt', 'r', encoding="utf-8") as f:
        ...
        for line in f:
            for word in line.split():
                time1 = time.time()
                contains_result = new_tree.contains(word)
                time2 = time.time()
